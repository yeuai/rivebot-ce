...
class IobParser {
   ...
   * @returns {Array|*}
   */
  matchEntities(zippedWords: string[][]) {
  ...
  generateEntitiesFromTags(tags: string[]) {
    let start = 0;
    const vEntities = _.map(tags, (tag, i) => {
      const entity = [];
      entity[0] = 'T' + (i + 1);
      entity[1] = tag[1];
      entity[2] = start;
      entity[3] = start + tag[0].length;
      entity[4] = tag[0];
